...
def main():
    ...
    input_ext, output_ext = splitext(input_file)[1], splitext(output_file)[1]
    if input_ext.lower() not in [".jpg", ".jpeg", ".png"]:
        sys.exit("Invalid input")
    if output_ext.lower() not in [".jpg", ".jpeg", ".png"]:
    ...
    size = shirt.size
    character = ImageOps.fit(image,size)
    character.paste(shirt,shirt)
